...
// SOLUTION:
function solve(s) {
    const upperLetters = s.split('').filter((el) => el.toUpperCase() === el);
    const lowerLetters = s.split('').filter((el) => el.toLowerCase() === el);
    return upperLetters.length > lowerLetters.length ? s.toUpperCase() : s.toLowerCase();
}